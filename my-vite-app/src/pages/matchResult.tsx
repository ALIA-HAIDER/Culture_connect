...
// Mock data for fallback when no filtered locations are available
const fallbackCommunities = [
  {
    ...
    priceRange: "₹25K - ₹45K",
    safetyScore: 9,
    highlights: ["Tech Community", "International Food", "Art Galleries", "Co-working Spaces"],
    description: "A vibrant tech hub with diverse cultural communities and modern amenities.",
    vibes: ["Trendy & Modern", "Social & Community-Focused", "Diverse & Multicultural"],
    languages: ["English", "Hindi", "Tamil", "Kannada"],
    amenities: ["Metro Access", "High-Speed Internet", "Cafes with WiFi", "Delivery Services"]
  },
  {
    id: "2",
    ...
    description: "Creative neighborhood with a thriving arts scene and cultural diversity.",
    vibes: ["Artistic & Creative", "Vibrant & Energetic", "Social & Community-Focused"],
    languages: ["English", "Hindi", "Kannada"],
    amenities: ["Bus Routes", "Art Galleries", "Co-working Spaces", "Entertainment"]
  },
  {
    id: "3",
    ...
    description: "Perfect for families with excellent schools, parks, and safe environment.",
...
function MatchResult() {
  const { filterLocation, getLocation } = useLocationStore();
  const [isLoading, setIsLoading] = useState(true);
  
  // Transform store data to match component interface
  const transformLocationData = (location: any) => ({
    ...
    location: location.location || `${location.area || location.city}, ${location.city}`,
    matchPercentage: location.matchPercentage || location.matchScore * 10 || Math.floor(Math.random() * 20) + 80,
  ...
  // Use filtered locations from store, or fallback to mock data
  const matchedCommunities = filterLocation && filterLocation.length > 0 
    ? filterLocation.map(transformLocationData)
    : fallbackCommunities;

  useEffect(() => {
    // If no filtered locations, try to load all locations as fallback
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-16">
          ...
          <div className="text-center mb-16">
            ...
            <div className="inline-flex items-center gap-3 bg-gradient-to-r from-emerald-500/10 to-green-500/10 backdrop-blur-sm px-6 py-3 rounded-full border border-emerald-500/20 mb-8 shadow-lg shadow-emerald-500/5">
              ...
              <span className="text-emerald-300 text-sm font-medium tracking-wide">Perfect Matches Found</span>
            ...
            <h1
              className={title({
                size: "lg",
                class: "mb-8 bg-gradient-to-r from-primary-300 via-magenta-300 to-violet-300 bg-clip-text text-transparent leading-tight",
              })}
            >
            ...
            <p className="text-mutedGray-300 text-xl max-w-4xl mx-auto leading-relaxed mb-8">
              Based on your preferences, we've curated the most compatible communities where you'll feel 
              <span className="text-primary-300 font-medium"> right at home</span>. Each match is carefully scored based on your cultural, lifestyle, and location preferences.
            </p>
            ...
            <div className="flex flex-wrap justify-center gap-8 mb-12">
              <div className="text-center">
                ...
                <div className="text-mutedGray-400 text-sm">Perfect Matches</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent mb-1">
                  {Math.round(matchedCommunities.reduce((acc, comm) => acc + comm.matchPercentage, 0) / matchedCommunities.length)}%
                </div>
              ...
              <div className="text-center">
                <div className="text-3xl font-bold bg-gradient-to-r from-violet-400 to-purple-400 bg-clip-text text-transparent mb-1">
                  {matchedCommunities.reduce((acc, comm) => acc + comm.residents, 0)}
                </div>
                <div className="text-mutedGray-400 text-sm">Community Members</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {matchedCommunities.map((community: any, index: number) => (
              <div
                key={community.id}
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="absolute top-3 right-3 flex gap-2">
                    <button className="p-1.5 rounded-full bg-deepBlack-800/60 text-mutedGray-300 border border-primary-500/30 hover:border-primary-500/50 backdrop-blur-lg transition-all">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="bg-gradient-to-r from-emerald-500 to-green-500 text-white backdrop-blur-lg px-2 py-1 rounded-full text-xs font-bold">
          ...
          <div className="mt-16 text-center">
            <div className="bg-gradient-to-br from-mutedGray-500/10 to-mutedGray-400/5 backdrop-blur-xl rounded-3xl p-12 border border-mutedGray-400/10 shadow-2xl max-w-2xl mx-auto">
              <h3 className="text-2xl font-bold text-softWhite mb-4">Not seeing what you're looking for?</h3>
              <p className="text-mutedGray-300 mb-8">
                Refine your preferences to discover more communities that match your lifestyle and values.
              </p>
